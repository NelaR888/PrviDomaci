...
<script>
    ...
    $(function () {
        ...
        $('#forma').submit(e => {
            ...
            if (selIndex === -1) {
                $.post('server/vrsta_pice/create.php', { naziv }).then(res => {
                    res = JSON.parse(res);
                    if (!res.status) {
                        alert(res.error);
                    } else {
                ...
                $.post('server/vrsta_pice/update.php', { naziv, id: kategorije[selIndex].id }).then(res => {
                    res = JSON.parse(res);
                    if (!res.status) {
                        alert(res.error);
                    } else {
    ...
    function obrisi(id) {
        $.post('server/vrsta_pice/delete.php', { id }).then((res) => {
            res = JSON.parse(res);
            if (!res.status) {
                alert(res.error);
                return;
